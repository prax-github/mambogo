...
-- Comprehensive schema for payment processing, status tracking, and refund handling
...
CREATE INDEX IF NOT EXISTS idx_payments_pending ON payments(id, status, expires_at) 
    WHERE status = 'PENDING';
CREATE INDEX IF NOT EXISTS idx_payments_failed ON payments(id, status, created_at) 
    WHERE status = 'FAILED';
